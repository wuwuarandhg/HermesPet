...
**拖文件给 AI · 让 AI 自己按需读**：不是把整个 PDF 塞进 context，而是把**绝对路径**拼到 prompt 末尾，Claude / Codex 用自己的 Read / Bash 工具**按需读哪几页** —— 省 context、省 token、更精准。图片支持四路输入：**剪贴板粘贴 / 拖拽 / `⌘⇧J` 截屏 / Codex 直接生图**。
